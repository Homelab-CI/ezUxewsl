...
Function library for homelab-ci charts

Since a lot of the homelab-ci charts follow a similar pattern, this library was built to reduce maintenance cost between the charts that use it and try achieve a goal of being DRY.
...
- name: common
  version: 4.5.1
  repository: https://github.com/homelab-ci/ezUxewsl/
```

For more information, take a look at the [Docs](http://docs.homelab-ci.com/our-helm-charts/common-library/).
...
**Important**: When deploying an application Helm chart you can add more values from our common library chart [here](https://github.com/homelab-ci/ezUxewsl/tree/master/charts/stable/common)
...
| addons.codeserver | object | See values.yaml | The common library supports adding a code-server add-on to access files. It can be configured under this key. For more info, check out [our docs](http://docs.homelab-ci.com/our-helm-charts/common-library-add-ons/#code-server) |
...
| addons.vpn | object | See values.yaml | The common chart supports adding a VPN add-on. It can be configured under this key. For more info, check out [our docs](http://docs.homelab-ci.com/our-helm-charts/common-library-add-ons/#wireguard-vpn) |
...
| addons.vpn.wireguard.image.repository | string | `"ghcr.io/homelab-ci/wireguard"` | Specify the WireGuard image |
...
| persistence | object | See below | Configure persistence for the chart here. Additional items can be added by adding a dictionary key similar to the 'config' key. [[ref]](http://docs.homelab-ci.com/our-helm-charts/common-library-storage) |
...
- Support for `nfs` as a persistence type. [[ref](https://docs.homelab-ci.com/our-helm-charts/common-library-storage/#nfs-volume)].
...
- `externalTrafficPolicy` (when a value is specified) is now set for all Service types. (fixes https://github.com/homelab-ci/library-charts/issues/125)
...
- Added `secret` and `configMap` as persistence types. [[ref]](http://docs.homelab-ci.com/our-helm-charts/common-library-storage/).
...
- Support ability to specify the [mountPropagation](https://kubernetes.io/docs/concepts/storage/volumes/#mount-propagation) key under persistence items (https://github.com/homelab-ci/library-charts/issues/74).

#### Changed

- Changed the docstring for `persistence` to better reference [our documentation](http://docs.homelab-ci.com/our-helm-charts/common-library-storage/) regarding Storage options.

#### Fixed

- Fixed an issue where the default `repository` value for the Wireguard addon was incorrect (https://github.com/homelab-ci/library-charts/issues/69).
...
- Removed persistence examples from the `values.yaml` file. These will be documented instead in [our documentation](http://docs.homelab-ci.com/our-helm-charts/common-library/).
...
- Fixed an issue where the code-server addon ingress would reference the wrong service when multiple hosts were specified (https://github.com/homelab-ci/library-charts/issues/64).
